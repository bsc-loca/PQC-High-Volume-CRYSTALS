...
int main(int argc, char **argv)
{
  ...
  std::chrono::time_point<std::chrono::high_resolution_clock> start, stop;
...
#endif
#ifdef U280
    custom_crypto_kem_enc(ct, key_b, pk);
#else
    for (unsigned int p = 0; p < packn; p++) {
    ...
    etime.encrypt.total += std::chrono::duration_cast<std::chrono::nanoseconds>( stop- start).count();
#endif
    ...
    stop = std::chrono::high_resolution_clock::now();
    etime2.encrypt.total += std::chrono::duration_cast<std::chrono::nanoseconds>( stop- start).count();
#endif
    ...
    printf("==================================== Info ===========================================\n");
    printf("Kernel                    : CRYSTALS-Kyber encapsulation\n");
    printf("Security Level            : %u\n", KYBER_K);
    printf("Batch Size                : %u\n", packn);
#ifdef BENCH
    printf("Encapsulation FPGA Total  : %zu ns\n", etime.encrypt.total);
    printf("Encapsulation FPGA Region : %zu ns\n", etime.encrypt.region);
    printf("Encapsulation CPU  Total  : %zu ns\n", etime2.encrypt.total);
    printf("Encapsulation CPU  Region : %zu ns\n", etime2.encrypt.region);
#elif defined(TIME)
#ifdef U280
    printf("Encapsulation FPGA Total  : %zu ns\n", etime.encrypt.total);
    printf("Encapsulation FPGA Region : %zu ns\n", etime.encrypt.region);
