...
#ifdef U280
int custom_crypto_kem_dec(uint8_t *ss,
                          uint8_t *ct,
                          uint8_t *sk)
{
    call_kem_dec(ss, ct, sk);
#ifdef VERIFY
    for (unsigned int p = 0; p < packn; p++) {
        uint8_t ss2[KYBER_SSBYTES];
        crypto_kem_dec(ss2,
                       ct + p * KYBER_CIPHERTEXTBYTES,
                       sk + p * KYBER_SECRETKEYBYTES);
        if (memcmp(ss + p * KYBER_SSBYTES, ss2, KYBER_SSBYTES)) {
            printf("[Error] kem_dec computation: ss\n");
            for (unsigned int i = 0; i < KYBER_SSBYTES; i++) {
                printf("ss[%2d] = %3d instead of %3d\n", 
                       i,
                       *(ss + p * KYBER_SSBYTES + i),
                       ss2[i]);
            }
            releaseClResources();
