...
#endif
  unsigned int n;
  uint8_t seedbuf[3*SEEDBYTES + TRBYTES + 2*CRHBYTES];
...
#else
  memset(rnd, 0, SEEDBYTES);
#endif

  shake256(rhoprime, CRHBYTES, key, SEEDBYTES + SEEDBYTES + CRHBYTES);
...
rej:
  ...
  shake256_finalize(&state);
  shake256_squeeze(sig, CBYTES, &state);
                                  ...
                                  const uint8_t *sk)
{
  ...
  std::chrono::time_point<std::chrono::high_resolution_clock> start, stop;
#endif
  uint8_t *tr = (uint8_t *)malloc(packn * TRBYTES * sizeof(uint8_t));
  uint8_t *mu = NULL;
  ...
  for (unsigned int i = 0; i < packn; i++) {
      for (unsigned int j = 0; j < TRBYTES; j++)
          tr[i * TRBYTES + j] = sk[i * CRYPTO_SECRETKEYBYTES + 2 * SEEDBYTES + j];

      /* Compute CRH(tr, msg) */
      shake256_init(&state);
      shake256_absorb(&state, tr + i * TRBYTES, TRBYTES);
                       ...
                       const uint8_t *pk)
{
  ...
  shake256_finalize(&state);
  shake256_squeeze(c2, CBYTES, &state);
  for(i = 0; i < CBYTES; ++i)
    if(c[i] != c2[i])
      return -1;
